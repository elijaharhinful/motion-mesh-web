  ...
  const onSubmit = (data: FormData) => {
    login(data, {
      ...
      onError: (err: unknown) => {
        ...
        addToast({ type: 'error', title: 'Login failed', message: e?.message ?? 'Invalid credentials' });
      },
    });
  };
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/15 text-white placeholder-white/30 text-sm focus:outline-none focus:border-violet-500/60 focus:bg-white/8 transition-all"
              />
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/15 text-white placeholder-white/30 text-sm focus:outline-none focus:border-violet-500/60 transition-all pr-10"
                />
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full py-3 rounded-xl bg-violet-600 hover:bg-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-sm transition-all shadow-lg shadow-violet-500/20 mt-2"
            >
